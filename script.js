	...
	if(card.innerHTML == "" && memory_values.length < 2){  //check if innerHTML is empty && memory values arraay is less than 2
		...
		if(memory_values.length == 0){
			...
			if(memory_values[0] == memory_values[1]){    //check if both cards are a match
				card_flipped += 2;    //if cards are a match, accumulate card flipped
				// Clear both arrays
				memory_values = [];
        memory_card_ids = [];
				// Check to see if the whole board is cleared
				if(card_flipped == memory_array.length){    //check if tiles flipped is = to the memory board array length
				...
				function flip2Back(){
				    ...
				    card_1.style.background = 'url(bamoo.png) no-repeat';
            	    card_1.innerHTML = "";
				    card_2.style.background = 'url(bamoo.png) no-repeat';
            	    card_2.innerHTML = "";
				    // Clear both arrays
				    memory_values = [];
            memory_card_ids = [];
